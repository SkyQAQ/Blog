...
namespace Blog.Core.Common
{
    public class Cache
    {
        ...
        public Cache()
        {
            ...
            try
            {
                if (string.IsNullOrWhiteSpace(config.RS_ConnectString))
                    throw new ApplicationException("配置文件中未找到RedisServer的有效配置");
                connectionMultiplexer = ConnectionMultiplexer.Connect(config.RS_ConnectString);
        ...
        public T Get<T>(string key)
        {
            ...
            DateTime begin = DateTimeUtils.NowBeijing();
        ...
        {
            var timeSpan = cacheTime - DateTimeUtils.NowBeijing();
            var jsonData = GetJsonData(data, TimeOut, true);
            database.StringSet(key, jsonData, timeSpan);
        }
        ...
        /// <returns></returns>
        public long ListLeftPush(string key, string data)
        {
            return database.ListLeftPush(key, data);
        }
    }
}
